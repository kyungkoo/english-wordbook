...
class VocabularyPersistenceCodecTest {
    ...
    fun restoresBooksWordsAndMemorizedState() {
        ...
        val restored =
            VocabularyPersistenceCodec.decode(VocabularyPersistenceCodec.encode(memorized))
    ...
    fun preservesNewlinesTabsAndSeparatorsInsideText() {
        val original =
            VocabularyUiState()
                .saveScannedBook(
                    "시험\t단어장",
                    listOf(ScannedWord("look-up", "찾다\n확인하다", "to search | check")),
                )

        val restored =
            VocabularyPersistenceCodec.decode(VocabularyPersistenceCodec.encode(original))

...
class OcrVocabularyParserTest {
    ...
    fun matchesRowsByNearestRowAndColumnWithoutFallingBackToWrongMeaning() {
        ...
        assertEquals(listOf(VocabRow("tall", "키가 큰"), VocabRow("short", "작은")), rows)
    }

    @Test
    fun splitsCompactSingleSyllableKoreanMeaningsByDetectedColumns() {
        val latinLines = listOf(line("ear", x = 100, y = 150), line("neck", x = 260, y = 150))
        val koreanLines = listOf(line("귀목", x = 370, y = 150, left = 340, right = 400))

        val rows = OcrVocabularyParser.parseLinesForTest(latinLines, koreanLines, VocabMode.EN_KO)

        assertEquals(listOf(VocabRow("ear", "귀"), VocabRow("neck", "목")), rows)
    }

    @Test
    fun splitsMultipleTermsDetectedInOneOcrLine() {
        val latinLines = listOf(line("ear(s), neck", x = 180, y = 150, left = 90, right = 270))
        val koreanLines = listOf(line("귀목", x = 370, y = 150, left = 340, right = 400))

        val rows = OcrVocabularyParser.parseLinesForTest(latinLines, koreanLines, VocabMode.EN_KO)

        assertEquals(listOf(VocabRow("ear(s)", "귀"), VocabRow("neck", "목")), rows)
    }
    ...
    fun keepsMultiWordEnglishMeaningsInTheMeaningColumn() {
        ...
        assertEquals(
            listOf(VocabRow("brave", "not afraid of danger"), VocabRow("quick", "moving fast")),
            rows,
        )
    }
    ...
    fun keepsNameWhenItAppearsAsVocabularyTermInRows() {
        ...
        assertEquals(listOf(VocabRow("name", "이름"), VocabRow("date", "날짜")), rows)
    }
    ...
    private fun line(
        ...
        left: Int = x - 30,
        right: Int = x + 30,
    ): OcrVocabularyParser.RecognizedLine =

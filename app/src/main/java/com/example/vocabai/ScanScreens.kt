...
fun ScanResultScreen(
        ...
        LazyColumn(
            ...
            item {
                NotebookCard(
                    modifier = Modifier.fillMaxWidth(),
                    containerColor =
                        if (feedback.canSave) AppColors.PrimarySoft else AppColors.ReviewSoft,
                ) {
                    Column(
            ...
            if (editableWords.isEmpty()) {
                ...
                items(editableWords, key = { it.english + it.koreanMeaning + it.englishMeaning }) {
                    word ->
    ...
    editingWord?.let { word ->
        WordEditSheet(
            ...
            onSave = { updated ->
                editableWords = editableWords.map {
                    if (it == word)
                        ScannedWord(updated.english, updated.koreanMeaning, updated.englishMeaning)
                    else it
                }
                editingWord = null
            },

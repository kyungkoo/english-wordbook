...
object OcrVocabularyParser {
    ...
    fun parse(latinText: Text, koreanText: Text, mode: VocabMode): List<VocabRow> {
        ...
        logInfo(
            "ML Kit pairs=${pairs.joinToString(" | ") { "${it.term} => ${it.meaning}" }.takeForLog()}"
        )
        return pairs
    }
    ...
    private fun parseRecognizedLines(
        ...
        val termTokens =
            if (mode == VocabMode.EN_EN && englishColumns != null) {
                ...
                latinLines.flatMap { it.toVocabularyTerms(latinLines) }.sortedForReading()
            }
        ...
        val rawMeaningTokens =
            if (mode == VocabMode.EN_KO) {
                ...
                if (englishColumns != null) {
                    englishMeaningLines
                        .filter {
                            it.isNearColumn(englishColumns.meaningColumn, englishColumns.tolerance)
                        }
                        .sortedForReading()
                } else {
                    englishMeaningLines
                        .filterNot { candidate ->
                            termTokens.any { term ->
                                term.text.equals(candidate.text, ignoreCase = true)
                            }
                        }
                        .sortedForReading()
                }
        ...
        val columnFilteredTerms =
            termTokens.filterByColumn(layout.termColumn, layout.rowAxis, layout.rowTolerance * 5)
        val columnFilteredMeanings =
            meaningTokens.filterByColumn(
                layout.meaningColumn,
                layout.rowAxis,
                layout.rowTolerance * 5,
            )
    ...
    private fun Text.toRecognizedLines(): List<RecognizedLine> = textBlocks.flatMap { block ->
    ...
    private fun detectRowAxis(
        ...
        val avgNearestX =
            termTokens
                .map { term -> meaningTokens.minOf { kotlin.math.abs(it.x - term.x) } }
                .average()
        val avgNearestY =
            termTokens
                .map { term -> meaningTokens.minOf { kotlin.math.abs(it.y - term.y) } }
                .average()
    ...
    private fun extractRowsByTableLayout(
        ...
        val sortedTerms =
            termTokens.sortedWith(
                compareBy<RecognizedLine> { it.rowValue(layout.rowAxis) }
                    .thenBy { it.columnValue(layout.rowAxis) }
            )
        val sortedMeanings =
            meaningTokens.sortedWith(
                compareBy<RecognizedLine> { it.rowValue(layout.rowAxis) }
                    .thenBy { it.columnValue(layout.rowAxis) }
            )
        ...
        sortedTerms.forEachIndexed { termIndex, term ->
            val closeCandidates =
                sortedMeanings
                    ...
                    .filter { (_, meaning) ->
                        kotlin.math.abs(
                            meaning.rowValue(layout.rowAxis) - term.rowValue(layout.rowAxis)
                        ) <= layout.rowTolerance
                    }

            val selected = closeCandidates.minByOrNull { (_, meaning) ->
                meaning.distanceFrom(term, layout)
            }
    ...
    private data class EnglishColumns(
        val termColumn: Int,
        val meaningColumn: Int,
        val tolerance: Int,
    )
    ...
    private fun List<RecognizedLine>.filterByColumn(
        targetColumn: Int,
        rowAxis: OcrRowAxis,
        tolerance: Int,
    ): List<RecognizedLine> {
        ...
        val filtered = filter {
            kotlin.math.abs(it.columnValue(rowAxis) - targetColumn) <= tolerance
        }
        return filtered.ifEmpty { this }
    }

    private fun detectEnglishColumns(lines: List<RecognizedLine>): EnglishColumns? {
        val wordHeader = lines.firstOrNull {
            it.text.normalizedHeader() in setOf("word", "words", "term", "terms")
        }
        val meaningHeader = lines.firstOrNull {
            it.text.normalizedHeader() in setOf("meaning", "meanings", "definition", "definitions")
        }
        if (wordHeader != null && meaningHeader != null && wordHeader.x != meaningHeader.x) {
            ...
            return EnglishColumns(
                wordHeader.x,
                meaningHeader.x,
                (gap * 0.45).toInt().coerceAtLeast(70),
            )
        }
    ...
    private fun RecognizedLine.toVocabularyTerms(
        ...
        val match =
            Regex(
                    """^[0-9]+[.)]?\s+(.+?)(?:\s+(adjective|noun|verb|adverb|preposition|conjunction))?$""",
    ...
    private fun RecognizedLine.toKoreanMeaningOrNull(
        allLines: List<RecognizedLine>
    ): RecognizedLine? {
    ...
    private fun RecognizedLine.toEnglishMeaningOrNull(): RecognizedLine? {
        ...
        val withoutPos =
            withoutNumber
                .replace(
                    Regex(
                        """\b(adjective|noun|verb|adverb|preposition|conjunction)\b""",
                        RegexOption.IGNORE_CASE,
                    ),
                    "",
                )
    ...
    private fun splitCompactMeaningNearTerms(
        ...
        val nearbyTerms = terms.filter {
            kotlin.math.abs(it.rowValue(layout.rowAxis) - meaning.rowValue(layout.rowAxis)) <=
                layout.rowTolerance
        }
        ...
        return compact.mapIndexed { index, char ->
            meaning.copy(
                ...
                rowValue = nearbyTerms[index].rowValue(layout.rowAxis),
            )
        }
    }
    ...
    private fun recoverKoreanGapMeanings(
        ...
        sortedMeanings.forEachIndexed { meaningIndex, meaning ->
            ...
            if (compactTail.length !in 2..4 || !meaning.text.contains(Regex("""\s""")))
                ...
                sortedTerms
                    ...
                    .filter { (index, term) ->
                        index !in matchedMeanings &&
                            term.rowValue(layout.rowAxis) >
                                meaning.rowValue(layout.rowAxis) + layout.rowTolerance
                    }
    ...
    private fun RecognizedLine.copy(
        text: String,
        rowAxis: OcrRowAxis,
        rowValue: Int,
    ): RecognizedLine =
    ...
    private fun String.normalizedHeader(): String = lowercase().replace(Regex("""[^a-z가-힣]"""), "")

    private fun String.splitVocabularyTermParts(): List<String> =
        split(Regex("""\s*[,;·]\s*""")).map { it.trim() }.filter { it.isNotBlank() }

    private fun String.compactHangulText(): String = filter { it in '가'..'힣' }

    private fun String.compactHangulTail(): String =
        trim().split(Regex("""\s+""")).lastOrNull().orEmpty().compactHangulText()

    private fun String.hasOnlyHangulCompactToken(): Boolean {
        val trimmed = trim()
        return trimmed.isNotBlank() &&
            trimmed.none { it.isWhitespace() } &&
            trimmed.all { it in '가'..'힣' }
    }
    ...
    private fun RecognizedLine.isLikelyLabel(allLines: List<RecognizedLine>): Boolean {
        ...
        val topContentY =
            allLines
                .filterNot { it.text.normalizedHeader().all(Char::isDigit) }
                .minOfOrNull { it.y } ?: y
    ...
    private fun String.takeForLog(): String = replace("\n", "\\n").take(3500)
    ...
    private fun String.isPartOfSpeechNoise(): Boolean {
        ...
        if (
            normalized in setOf("adjective", "noun", "verb", "adverb", "preposition", "conjunction")
        )
            return true
        return setOf("adjective", "noun", "verb", "adverb", "preposition", "conjunction").any {
            partOfSpeech ->
            normalized.startsWith(partOfSpeech) && normalized.length <= partOfSpeech.length + 2
        }
    }
    ...
    private fun String.isEnglishMeaningCandidate(): Boolean {
        ...
        if (lower in setOf("adjective", "noun", "verb", "adverb", "preposition", "conjunction"))
            return false
        return normalized.any { it in 'a'..'z' || it in 'A'..'Z' }
    }
}

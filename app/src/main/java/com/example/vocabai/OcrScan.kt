...
private suspend fun scanInputImageToWords(inputImage: InputImage): List<ScannedWord> {
    ...
    val englishRows =
        OcrVocabularyParser.parse(latinText, koreanText, VocabMode.EN_EN).associateBy {
            it.term.lowercase()
        }
...
private suspend fun com.google.mlkit.vision.text.TextRecognizer.processImageAwait(
    image: InputImage
): MlKitText = suspendCancellableCoroutine { continuation ->
    process(image)

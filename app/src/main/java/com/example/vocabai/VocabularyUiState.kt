...
data class ScannedWord(val english: String, val koreanMeaning: String, val englishMeaning: String)
...
data class VocabularyUiState(
    ...
    fun saveScannedBook(
        ...
        val entries = scanWords.mapIndexed { index, word ->
            WordEntry(
    ...
    fun reviewWordsFor(bookId: String): List<WordEntry> =
        wordsFor(bookId).filterNot { it.memorized }
    ...
    fun renameBook(
        ...
        copy(
            books =
                books.map {
                    if (it.id == bookId) it.copy(title = title.ifBlank { "새 단어장" }, updatedAt = now)
                    else it
                }
        )
    ...
    fun markMemorizedAndAdvance(bookId: String, wordId: String): VocabularyUiState =
        copy(
            words =
                words.map {
                    if (it.bookId == bookId && it.id == wordId) it.copy(memorized = true) else it
                }
        )
    ...
    fun markNeedsReview(bookId: String, wordId: String): VocabularyUiState =
        copy(
            words =
                words.map {
                    if (it.bookId == bookId && it.id == wordId) it.copy(memorized = false) else it
                }
        )

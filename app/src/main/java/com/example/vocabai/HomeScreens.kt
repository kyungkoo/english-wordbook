...
@Composable
fun HomeScreen(state: VocabularyUiState, onOpenScan: () -> Unit, onOpenBook: (String) -> Unit) {
    Scaffold(
        topBar = { WordTopBar(title = "WordNote", actionLabel = "스캔", onAction = onOpenScan) },
        ...
        LazyColumn(
            ...
            item { TodayReviewCard(state = state, onOpenScan = onOpenScan) }
...
private fun TodayReviewCard(state: VocabularyUiState, onOpenScan: () -> Unit) {
    ...
    NotebookCard(modifier = Modifier.fillMaxWidth(), containerColor = AppColors.PrimarySoft) {
        Column(
            ...
            Text(
                if (summary.total == 0) "시험지를 스캔해서 첫 단어장을 만들어 보세요."
                else "${summary.review}개 단어가 복습을 기다리고 있어요.",
        ...
        Column(
            ...
            Row(
                ...
                Column(
                    ...
                    Text(
                        "최근 수정 ${book.updatedAt.formatDate()}",
                        color = AppColors.Muted,
                ...
                ProgressPill(
                    summary.reviewLabel,
                    accent = if (summary.review == 0) AppColors.Primary else AppColors.Review,
                )
            }

...
fun FlipCardScreen(
    ...
    Column(
        ...
        Text(
            if (flipped) "뜻을 확인했으면 상태를 골라 주세요." else "카드를 탭하면 뜻을 볼 수 있어요.",
            color = AppColors.Muted,
        )
        if (flipped) {
            ...
            OutlinedButton(onClick = ::advanceWithoutMarking, shape = ButtonShape) { Text("다음 단어") }
        }
    }
}
        ...
        NotebookCard(modifier = Modifier.fillMaxWidth(), containerColor = AppColors.PaperDeep) {
            Text(
                game.selectedTiles
                    ...
                    .ifBlank { List(game.target.length) { "_" }.joinToString(" ") },
        ...
        if (game.isCorrect) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                OutlinedButton(
                    ...
                    enabled = game.selectedTiles.isNotEmpty(),
                    shape = ButtonShape,
                ) {
                    Text("지우기")
                }
                OutlinedButton(
                    onClick = { game = game.reset() },
                    enabled = game.selectedTiles.isNotEmpty(),
                    shape = ButtonShape,
                ) {
                    Text("초기화")
...
private fun StudyModeHeader(
    ...
    Column(modifier = Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        Row(
            ...
            ProgressPill(
                if (reviewOnly) "미암기 모드" else "전체 모드",
                accent = if (reviewOnly) AppColors.Review else AppColors.Primary,
            )
        }
        IconTextButton(
